...
import { useOrder, useUpdateOrder } from "@/hooks/useOrderAdmin";
import moment from "moment";
import React from "react";

const Page = () => {
  const { data: allOrders = [] } = useOrder();
  ...
  const orders = allOrders.filter((order) => moment(order.orderedTime).isSame(today, 'day'));
  ...
  const handleStart = (id: string) => {
    updateOrder({ orderId: id, status: "In the Kitchen" });
  };
  const handleSentToDelivery = (id: string) => {
    updateOrder({ orderId: id, status: "Sent to Delivery" });
  };
          ...
          <div className="flex flex-col gap-3 w-full h-full p-5">
            ...
            <div className="flex flex-row gap-3 overflow-x-auto scrollbar-hide">
              {beingPrepared.map((item) => (
                <div
                  key={item._id}
                  className="flex flex-col items-center bg-components text-black rounded-xl h-[10rem] w-[20rem] justify-between transition-all duration-300 ease-in-out hover:opacity-60 hover:scale-[1.02]"
                >
                  ...
                  <Button onClick={() => handleSentToDelivery(item._id ?? "")}>
                    Sent To Delivery
                  </Button>
                </div>
              ))}
          ...
          <div className="left w-[70%] h-[20rem] bg-components rounded-xl p-2">
            ...
            <div className="w-full h-[16rem] mb-2 flex flex-col gap-4 overflow-y-auto scrollbar-hide">
              {sentToDelivery.map((item, index) => (
                <div
                  key={index}
                  className="order_item flex flex-row gap-3 justify-between items-center transition-all duration-300 ease-in-out hover:opacity-60 hover:scale-[1.02]"
                >
                  <div className="flex flex-row gap-2 h-[3rem] items-center justify-between w-full px-2">
                    <span>{index + 1}</span>
          ...
          <div className="right w-[30%] h-[20rem] bg-components rounded-xl p-2">
            ...
            <div className="h-[17rem] flex flex-col gap-3 overflow-y-auto scrollbar-hide">
              {ingredientsList.map((item, index) => (
                <div
                  key={index}
                  className="flex flex-row gap-2 mx-auto transition-all duration-300 ease-in-out hover:opacity-60 hover:scale-[1.02]"
                >
                  <h1>{item.name}</h1>
